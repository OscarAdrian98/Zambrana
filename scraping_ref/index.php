...
<script>const API_BASE_URL = <?php echo json_encode(rtrim(getenv('SCRAPING_API_URL') ?: 'http://127.0.0.1:8000', '/'), JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;</script>
</head>
<body>
...
<script>
    $(document).ready(function () {
        ...
        $('#searchForm').on('submit', function (e) {
            ...
            const resultados = [];
            const eventSource = new EventSource((API_BASE_URL + `/buscar?reference=${encodeURIComponent(reference)}`)); // Poner ip de la máquina donde se ejecute el script de Python
            ...
            eventSource.onmessage = function (event) {
                ...
                if (data.completed) {
                    ...
                    if (resultados.length > 0) {
                        const mxdemo = resultados.find(p => p.competencia === 'MX demo');
                        ...
                        if (cheapest) {
                            ...
                            const mxdemoPrice = parseFloat(mxdemo?.final_price.replace(',', '.').replace('€', '')) || Infinity;
                            const cheapestPrice = parseFloat(cheapest.final_price.replace(',', '.').replace('€', '')) || Infinity;
                            const diff = Math.max(0, mxdemoPrice - cheapestPrice);

                            $('#resumen').html(`
                                <p>Lo tiene más barato: <strong>${cheapest.competencia || '-'}</strong> por <strong>${cheapest.final_price || '-'}</strong>.</p>
                                <p>La diferencia con MX demo es de <strong>${diff.toFixed(2)}€</strong>.</p>
                            `);
                        }
                    }
                    ...
                    if (similarity >= 0.7) {
                        ...
                        const competenciaClass = product.competencia === 'MX demo' ? 'mxdemo' : '';
                        const row = `
        ...
        function descargarExcel(resultados, reference) {
            $.ajax({
                url: (API_BASE_URL + '/descargar'), // Poner ip de la máquina donde se ejecute el script de Python
                type: 'POST',

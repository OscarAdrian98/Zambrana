...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[origin for origin in os.environ.get("APP_ALLOWED_ORIGINS", "").split(",") if origin],
...
async def analisis_ventas(fecha_desde: str = Query(...), fecha_hasta: str = Query(...)):
    try:
        ...
        response = requests.get(f"{os.environ.get('INVENTARIO_API_URL', 'http://127.0.0.1:5002')}/get_data?tipo=ventas&fecha_desde={fecha_desde}&fecha_hasta={fecha_hasta}&ventas=individual")
        ...
        # Obtener stock
        try:
            stock_response = requests.get(f"{os.environ.get('INVENTARIO_API_URL', 'http://127.0.0.1:5002')}/get_data?tipo=stock")

...
class BitCompatibleBoolean(TypeDecorator):
    """BIT(1) en MySQL/MariaDB y BOOLEAN en SQLite."""
    ...
    def load_dialect_impl(self, dialect):
        if dialect.name in {"mysql", "mariadb"}:
            return dialect.type_descriptor(BIT(1))
        return dialect.type_descriptor(Boolean())

    def process_bind_param(self, value, dialect):
        if value is None:
            return None
        return 1 if _coerce_boolean(value) else 0
...
class RegistroHorario(db.Model):
    ...
    creado_por_admin = db.Column(BitCompatibleBoolean(), default=False)
    eliminado = db.Column(BitCompatibleBoolean(), default=False)
    origen = db.Column(db.String(20), default="Tienda")

    fichaje = db.relationship(
        "Fichaje",
        backref=db.backref(
            "registros", lazy="joined", order_by="RegistroHorario.timestamp"
        ),
    )
...
class Ausencia(db.Model):
    ...
    hora_desde = db.Column(db.Time, nullable=True)
    hora_hasta = db.Column(db.Time, nullable=True)
